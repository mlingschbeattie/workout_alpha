...
// best-effort helper: swallow duplicate errors on createMany, etc.
async function ignoreDuplicates<T>(fn: () => Promise<T>) {
  try {
    return await fn();
  } catch (e: any) {
...
async function main() {
  // --- Users (deterministic IDs so we can upsert safely)
  await prisma.user.upsert({
    where: { id: 'usr:coach-carter' },
    update: {},
    create: { id: 'usr:coach-carter', name: 'Coach Carter', role: 'COACH' },
  });

  await prisma.user.upsert({
    where: { id: 'usr:athlete-adams' },
    update: {},
    create: { id: 'usr:athlete-adams', name: 'Athlete Adams', role: 'ATHLETE' },
  });

  // --- Exercises (IDs are deterministic; ignore duplicates on re-run)
  ...
  // --- Plans (upsert by deterministic IDs)
  await prisma.plan.upsert({
    where: { id: 'plan:dt-separation-4w-template' },
    update: {},
    create: {
  ...
  await prisma.plan.upsert({
    where: { id: 'plan:dt-separation-4w-progress' },
    update: {},
    create: {
  ...
  console.log('Seeded users, exercises, and plans.');
}
